...
app.get(
  '/og/:title/:subtitle/:url/:protocol/:font/:image/:color/data.png',
  (req, res) => {
    // res.send(req.params);
    ...
    console.log(
      `${count}: {title: '${req.params.title}', subtitle: '${req.params.subtitle}', url: '${req.params.url}', image: '${req.params.image}', font: '${req.params.font}'}`
    );
    ...
    connection.query(
      `INSERT INTO uses (title, subtitle, protocol, url, font, color, background) VALUES (:title, :subtitle, :protocol, :url, :font, :color, :background)`,
      {
